...
async function listarTodas(req, res) {
  try {
    ...
    return res.status(500).json({ sucesso: false, erro: "Erro ao listar categorias" });
  }
}
...
async function remover(req, res) {
  try {
    ...
    return res.status(500).json({ sucesso: false, erro: "Erro ao remover categoria" });
  }
}
...
async function buscarPorIdade(req, res) {
  try {
    ...
    const categoria = await categoriasService.buscarPorIdade(idade, organizacaoId);

    return res.json({ sucesso: true, data: categoria });
  } catch (err) {
    logger.error("[categoriasController] buscarPorIdade", err);
    return res.status(500).json({ sucesso: false, erro: "Erro ao buscar categoria por idade" });
  }
}
