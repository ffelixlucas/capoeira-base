...
async function criarLembrete(dados) {
  ...
  const [result] = await db.query(
    `INSERT INTO lembretes (titulo, descricao, prioridade, data, criado_por)
     VALUES (?, ?, ?, ?, ?)`,
    [titulo, descricao, prioridade, data, criado_por]
  );
...
async function atualizarLembrete(id, dados) {
  ...
  await db.query(
    `UPDATE lembretes SET titulo = ?, descricao = ?, prioridade = ?, status = ?, data = ? WHERE id = ?`,
