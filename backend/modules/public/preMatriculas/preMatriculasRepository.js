...
async function verificarCpfExistente(cpf, organizacao_id) {
  try {
    ...
    if (existe) {
      logger.warn(
        `[preMatriculasRepository] CPF duplicado detectado: ${cpf} (org ${organizacao_id})`
      );
    }
    return existe;
  } catch (err) {
...
async function listarPendentes(organizacaoId) {
  try {
    ...
    logger.debug(
      `[preMatriculasRepository] org ${organizacaoId} - ${rows.length} pré-matrículas pendentes encontradas.`
    );
...
async function atualizarStatus(id, novoStatus, organizacaoId) {
  try {
    ...
    if (result.affectedRows === 0) {
      logger.warn(
        `[preMatriculasRepository] Nenhum registro atualizado (id ${id}, org ${organizacaoId})`
      );
      throw new Error("Pré-matrícula não encontrada para esta organização.");
    }

    logger.info(
      `[preMatriculasRepository] Status atualizado → ${novoStatus} (ID ${id}, org ${organizacaoId})`
    );
  } catch (err) {
    logger.error(
...
async function deletar(id, organizacao_id) {
  try {
    ...
    if (result.affectedRows > 0) {
      logger.info(
        `[preMatriculasRepository] Pré-matrícula ${id} removida com sucesso (org ${organizacao_id})`
      );
      return true;
    } else {
      logger.warn(
        `[preMatriculasRepository] Nenhuma pré-matrícula removida (id ${id}, org ${organizacao_id})`
      );
      return false;
    }
  } catch (err) {
...
async function buscarPorId(id, organizacao_id) {
  try {
    ...
    if (rows.length > 0) {
      logger.debug(
        `[preMatriculasRepository] org ${organizacao_id} - pré-matrícula encontrada (id ${id})`
      );
      return rows[0];
    } else {
      logger.warn(
        `[preMatriculasRepository] org ${organizacao_id} - pré-matrícula não encontrada (id ${id})`
      );
      return null;
    }
  } catch (err) {
    logger.error(
      `[preMatriculasRepository] Erro ao buscar pré-matrícula (id ${id}, org ${organizacao_id}): ${err.message}`
    );
    throw err;
  }
}
