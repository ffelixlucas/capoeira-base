...
const { buscarInscricaoPendente, criarInscricaoPendente, atualizarInscricaoComPix, atualizarInscricaoParaPago, atualizarInscricaoPendente, buscarInscricaoComEvento, verificarInscricaoPaga, buscarValorEvento, verificarEncerramentoInscricao, } = require("./inscricoesRepository");
const { enviarEmailConfirmacao, enviarEmailPendente, } = require("../../../services/emailService.js");
...
const gerarPagamentoPixService = async (dadosFormulario) => {
    const { cpf, responsavel_documento, nome, apelido, valor, evento_id } = dadosFormulario;
    ...
    // 🔒 Validar valor
    const valorNum = parseFloat(dadosFormulario.total_amount || dadosFormulario.valor);
    ...
    const emailPagador = dadosFormulario.responsavel_email || dadosFormulario.email;
...
const gerarPagamentoCartaoService = async (dadosFormulario) => {
    try {
        ...
        const { token, payment_method_id, installments = 1, issuer_id, cpf, responsavel_documento, nome, apelido, email, telefone, valor, evento_id, responsavel_nome, responsavel_email, } = dadosFormulario;
        // ⛔ Bloqueio de inscrições após data limite
        const encerrado = await verificarEncerramentoInscricao(evento_id);
        if (encerrado) {
            throw new Error("As inscrições para este evento estão encerradas.");
        ...
        if (result.status === "approved") {
            ...
            const taxa_percentual = valorBase > 0 ? ((taxa_valor / valorBase) * 100).toFixed(2) : 0;
            ...
            const inscricaoCompleta = await buscarInscricaoDetalhadaService(inscricaoId);
...
async function fetchInstallments({ amount, bin = null, payment_method_id = null, issuer_id = null, }) {
    ...
    if (issuer_id)
        url.searchParams.append("issuer.id", issuer_id);
...
const calcularParcelasService = async ({ amount, payment_method_id = null, issuer_id = null, bin = null, }) => {
    try {
        ...
        console.error("❌ Erro Service calcularParcelasService:", err?.response?.data || err?.message || err);
...
const processarWebhookService = async (payload) => {
    ...
    try {
        ...
        logger.error("Erro no webhook do Mercado Pago:", err?.response?.data || err);
    }
};
...
const gerarPagamentoBoletoService = async (dadosFormulario) => {
    try {
        ...
        const { cpf, responsavel_documento, nome, apelido, email, telefone, valor, evento_id, responsavel_nome, responsavel_email, } = dadosFormulario;
        // ⛔ Bloqueio de inscrições após data limite
        const encerrado = await verificarEncerramentoInscricao(evento_id);
        if (encerrado) {
            throw new Error("As inscrições para este evento estão encerradas.");
        ...
        const inscricao = {
            ...
            date_of_expiration: result.date_of_expiration || null,
            status_detail: result.status_detail,
            codigo_inscricao: `GCB-${new Date().getFullYear()}-EVT${evento_id}-${String(inscricaoId).padStart(4, "0")}`,
            nome,
            apelido,
            email,
            telefone,
...
async function getValoresEvento(eventoId) {
    ...
    if (!evento) {
        logger.error("[inscricoesService.getValoresEvento] evento não encontrado:", eventoId);
        throw new Error("Evento não encontrado");
    }
    logger.debug("[inscricoesService.getValoresEvento] valor base:", evento.valor);
    // ✅ usa direto a função que retorna { pix, cartao, boleto }
    const valores = calcularValores(evento.valor);
    logger.debug("[inscricoesService.getValoresEvento] valores calculados:", valores);
    return valores;
}
module.exports = {
