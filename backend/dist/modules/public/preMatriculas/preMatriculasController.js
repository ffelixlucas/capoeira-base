...
async function criarPreMatricula(req, res) {
    try {
        ...
        if (req.params.slug) {
            ...
            logger.debug(`[preMatriculasController] Slug detectado na rota: ${req.params.slug}`);
        }
        ...
        logger.info(`[preMatriculasController] Nova pré-matrícula recebida (${dados.organizacao_id ? "org " + dados.organizacao_id : "via slug"})`);
        const resultado = await preMatriculasService.criarPreMatricula(dados);
        return res.status(201).json(resultado);
    }
    catch (err) {
        logger.error("[preMatriculasController] Erro ao criar pré-matrícula:", err.message);
        return res
            .status(400)
...
async function listarPendentes(req, res) {
    try {
        ...
        logger.error("[preMatriculasController] Erro ao listar pendentes:", err.message);
        return res
            .status(400)
...
async function atualizarStatus(req, res) {
    ...
    try {
        logger.info(`[preMatriculasController] org ${organizacao_id} - requisição recebida para atualizar status da pré ${id} → ${status}`);
        // 🎯 Encaminha tudo pro service (ele já trata aprovado/rejeitado internamente)
        const resultado = await preMatriculasService.atualizarStatus(id, status, organizacao_id);
        logger.info(`[preMatriculasController] org ${organizacao_id} - status ${status} processado com sucesso`);
        return res.json({
            sucesso: resultado?.sucesso ?? true,
            mensagem: resultado?.mensagem || `Status atualizado para ${status} com sucesso.`,
        });
    }
    catch (err) {
        logger.error(`[preMatriculasController] org ${organizacao_id} - erro ao atualizar status da pré ${id}:`, err.message);
...
async function getGrupo(req, res) {
    try {
        ...
        const grupo = await preMatriculasService.buscarGrupoPorOrganizacaoId(organizacaoId);
        return res.json({ grupo });
    }
    catch (err) {
        logger.error("[preMatriculasController] Erro ao buscar grupo:", err.message);
...
async function detectarTurmaPorIdade(req, res) {
    try {
        ...
        logger.debug(`[preMatriculasController] Detectando turma para slug=${slug} idade=${idade}`);
        ...
        logger.error("[preMatriculasController] Erro ao detectar turma:", err.message);
...
async function listarGraduacoesPorCategoriaPublic(req, res) {
    try {
        ...
        logger.debug(`[preMatriculasController] Buscando graduações (slug=${slug}, categoria=${categoriaId})`);
        const graduacoes = await preMatriculasService.listarGraduacoesPorCategoriaPublic({
            slug,
            categoriaId,
        });
        ...
        logger.error("[preMatriculasController] Erro ao listar graduações públicas:", err.message);
        return res
            .status(400)
