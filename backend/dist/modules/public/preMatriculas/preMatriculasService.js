...
const { gerarEmailPreMatriculaAdmin, } = require("../../../services/templates/preMatriculaAdmin");
    ...
    try {
        ...
        if (dados.slug && !dados.organizacao_id) {
            try {
                ...
                logger.debug(`[preMatriculasService] Organização resolvida via slug "${dados.slug}" → id ${orgId}`);
            }
            catch (err) {
        ...
        if (dados.imagemBase64) {
            try {
                ...
                logger.info(`[preMatriculasService] Upload concluído → ${destinoOriginal}`);
                ...
                if (caminhoResizedEncontrado) {
                    ...
                    logger.info(`[preMatriculasService] Imagem redimensionada usada → ${caminhoResizedEncontrado}`);
                    // 🧹 remove o original
                    try {
                        await fileOriginal.delete();
                        logger.debug(`[preMatriculasService] Original removido: ${destinoOriginal}`);
                    }
                    catch (err) {
                        logger.warn(`[preMatriculasService] Falha ao remover original (${destinoOriginal}): ${err.message}`);
                    }
                }
                else {
                    ...
                    logger.warn(`[preMatriculasService] Redimensionado não encontrado; usando original (${destinoOriginal})`);
                }
            }
            catch (err) {
        ...
        const cpfDuplicado = await preMatriculasRepository.verificarCpfExistente(dados.cpf, dados.organizacao_id);
        if (cpfDuplicado) {
            logger.warn(`[preMatriculasService] CPF duplicado bloqueado: ${dados.cpf}`);
        ...
        const cpfEmAluno = await preMatriculasRepository.verificarCpfEmAlunos(dados.cpf, dados.organizacao_id);
        if (cpfEmAluno) {
            logger.warn(`[preMatriculasService] CPF já pertence a um aluno ativo: ${dados.cpf}`);
        ...
        logger.info(`[preMatriculasService] Pré-matrícula criada com sucesso (ID ${id})`);
        // ✉️ Envio de e-mails
        ...
        (async () => {
            try {
                ...
                const orgInfo = await preMatriculasRepository.buscarGrupoPorOrganizacaoId(dados.organizacao_id);
                const nomeInstituicao = orgInfo?.nome_fantasia || orgInfo?.nome || "Capoeira Base";
                ...
                const emailsAdmin = (await notificacaoService.getEmails(dados.organizacao_id ?? null, "matricula")) || [];
                // 🔎 Buscar a pré-matrícula completa (com nomes de categoria e graduação)
                const preCompleta = await preMatriculasRepository.buscarPorId(id, dados.organizacao_id);
                if (!Array.isArray(emailsAdmin) || emailsAdmin.length === 0) {
                    logger.warn(`[preMatriculasService] org ${dados.organizacao_id} - nenhum e-mail admin configurado para tipo 'matricula'`);
                    return;
                }
                ...
                logger.info(`[preMatriculasService] org ${dados.organizacao_id} - e-mails enviados (modo assíncrono)`);
            }
            catch (emailErr) {
                logger.error("[preMatriculasService] Erro no envio assíncrono de e-mails:", emailErr.message);
            }
        })();
...
async function atualizarStatus(id, status, organizacaoId) {
    try {
        logger.debug(`[preMatriculasService] org ${organizacaoId} - iniciando atualização de status da pré ${id} → ${status}`);
        // 🧩 Atualiza o status no banco
        await preMatriculasRepository.atualizarStatus(id, status, organizacaoId);
        logger.info(`[preMatriculasService] org ${organizacaoId} - status atualizado para ${status} (pré ${id})`);
        // ⚙️ Quando aprovado → cria aluno e matrícula real
        if (status === "aprovado") {
            logger.debug(`[preMatriculasService] org ${organizacaoId} - status aprovado → iniciando criação de matrícula`);
            const pre = await preMatriculasRepository.buscarPorId(id, organizacaoId);
            if (!pre) {
                logger.warn(`[preMatriculasService] org ${organizacaoId} - pré-matrícula ${id} não encontrada ao tentar aprovar`);
            ...
            logger.info(`[preMatriculasService] org ${organizacaoId} - pré-matrícula ${id} aprovada e removida após criação de matrícula`);
        }
        // ⚠️ Quando rejeitado → exclui foto, registro e envia e-mail
        if (status === "rejeitado") {
            logger.debug(`[preMatriculasService] org ${organizacaoId} - status rejeitado → iniciando limpeza e notificação`);
            const pre = await preMatriculasRepository.buscarPorId(id, organizacaoId);
            if (!pre) {
                logger.warn(`[preMatriculasService] org ${organizacaoId} - pré-matrícula ${id} não encontrada ao rejeitar`);
                return { sucesso: false, erro: "Pré-matrícula não encontrada." };
            }
            ...
            if (pre.foto_url) {
                try {
                    ...
                    const filePath = decodeURIComponent(pre.foto_url.split(`${bucket.name}/`)[1].split("?")[0]);
                    ...
                    logger.debug(`[preMatriculasService] Tentando deletar possíveis caminhos:`);
                    for (const c of candidatos)
                        logger.debug(`→ ${c}`);
                    // 🧹 Deleta todos silenciosamente (mesmo se não existirem)
                    await Promise.allSettled(candidatos.map((path) => bucket.file(path).delete()));
                    logger.info(`[preMatriculasService] org ${organizacaoId} - fotos removidas do Firebase (${nomeArquivo})`);
                }
                catch (err) {
                    logger.warn(`[preMatriculasService] org ${organizacaoId} - falha ao excluir fotos do Firebase (${id}): ${err.message}`);
                }
            }
            // 🧹 2. Remove registro da tabela
            await preMatriculasRepository.deletar(id, organizacaoId);
            logger.info(`[preMatriculasService] org ${organizacaoId} - pré-matrícula ${id} removida do banco após rejeição`);
            ...
            if (pre.email) {
                ...
                logger.info(`[preMatriculasService] org ${organizacaoId} - e-mail de recusa enviado com sucesso (pré ${id})`);
            }
            else {
                logger.warn(`[preMatriculasService] org ${organizacaoId} - pré ${id} sem e-mail, recusa não enviada`);
            }
        }
        return { sucesso: true, mensagem: `Status atualizado para ${status}` };
    }
    catch (err) {
        logger.error(`[preMatriculasService] Erro ao atualizar status (pré ${id}):`, err.message);
        return {
            sucesso: false,
...
async function buscarGrupoPorOrganizacaoId(organizacaoId) {
    try {
        return await preMatriculasRepository.buscarGrupoPorOrganizacaoId(organizacaoId);
    }
    catch (err) {
...
async function deletarPreMatricula(id, organizacao_id) {
    try {
        ...
        if (removida) {
            logger.info(`[preMatriculasService] org ${organizacao_id} - pré-matrícula ${id} removida do banco (imagem mantida)`);
        }
        else {
            logger.warn(`[preMatriculasService] org ${organizacao_id} - tentativa de remoção sem sucesso (id ${id})`);
        }
        return removida;
    }
    catch (err) {
        logger.error(`[preMatriculasService] Erro ao deletar pré-matrícula ${id}:`, err.message);
        throw err;
    }
}
async function deletarPreMatriculaComImagem(id, organizacao_id) {
    try {
        logger.debug(`[preMatriculasService] org ${organizacao_id} - iniciando exclusão com imagem (pré ${id})`);
        // 🔍 Busca via repository
        const pre = await preMatriculasRepository.buscarPorId(id, organizacao_id);
        if (!pre) {
            logger.warn(`[preMatriculasService] org ${organizacao_id} - pré-matrícula ${id} não encontrada`);
            return false;
        }
        ...
        if (pre.foto_url) {
            try {
                ...
                if (match && match[1]) {
                    ...
                    if (exists) {
                        await file.delete();
                        logger.info(`[preMatriculasService] org ${organizacao_id} - imagem removida do Firebase: ${caminhoArquivo}`);
                    }
                    else {
                        logger.warn(`[preMatriculasService] org ${organizacao_id} - imagem não encontrada no Firebase: ${caminhoArquivo}`);
                    }
                }
                else {
                    logger.warn(`[preMatriculasService] org ${organizacao_id} - caminho da imagem inválido: ${pre.foto_url}`);
                }
            }
            catch (err) {
                logger.error(`[preMatriculasService] org ${organizacao_id} - erro ao remover imagem Firebase: ${err.message}`);
            }
        }
        else {
            logger.debug(`[preMatriculasService] org ${organizacao_id} - pré-matrícula ${id} sem imagem para remover`);
        }
        // 🗑️ Apagar do banco
        const removida = await preMatriculasRepository.deletar(id, organizacao_id);
        if (removida) {
            logger.info(`[preMatriculasService] org ${organizacao_id} - pré-matrícula ${id} removida (imagem e banco)`);
        }
        return removida;
    }
    catch (err) {
        logger.error(`[preMatriculasService] Erro ao deletar pré-matrícula ${id} com imagem: ${err.message}`);
        throw err;
    }
}
/**
...
async function detectarTurmaPorIdade({ slug, idade }) {
    try {
        logger.debug(`[preMatriculasService] Detectando turma para slug=${slug} idade=${idade}`);
        ...
        logger.debug(`[preMatriculasService] Organização resolvida → id ${organizacaoId}`);
  ...
  `, [organizacaoId, idade, idade]);
        if (!rows.length) {
            logger.warn(`[preMatriculasService] Nenhuma turma encontrada para idade ${idade} (org ${organizacaoId})`);
            return null;
        }
        const turma = rows[0];
        logger.info(`[preMatriculasService] Turma detectada → ${turma.turma_nome} | Categoria: ${turma.categoria_nome}`);
        return turma;
    }
    catch (err) {
        logger.error(`[preMatriculasService] Erro ao detectar turma por idade: ${err.message}`);
        throw err;
    }
}
/* -------------------------------------------------------------------------- */
...
async function listarGraduacoesPorCategoriaPublic({ slug, categoriaId }) {
    try {
        logger.debug(`[preMatriculasService] Listando graduações públicas (slug=${slug}, categoria=${categoriaId})`);
        ...
        const graduacoes = await preMatriculasGraduacoesRepository.listarGraduacoesPublic({
            categoriaId,
            organizacaoId,
        });
        return graduacoes;
        ...
        logger.error("[preMatriculasService] Erro ao listar graduações públicas:", err.message);
        throw err;
    }
}
module.exports = {
