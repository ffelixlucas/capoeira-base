...
async function getPorTipo(req, res) {
    try {
        ...
        logger.debug(`[notificacaoDestinosController] org ${organizacaoId} - retornando ${lista.length} registros (${tipo})`);
...
async function post(req, res) {
    try {
        ...
        logger.info(`[notificacaoDestinosController] org ${organizacaoId} - notificação criada (${email})`);
...
async function del(req, res) {
    try {
        ...
        await service.deletar(id, organizacaoId);
        logger.warn(`[notificacaoDestinosController] org ${organizacaoId} - notificação removida id ${id}`);
