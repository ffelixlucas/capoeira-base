...
async function listarPorCategoria(categoriaId, organizacaoId) {
    ...
    const [rows] = await db.execute(`SELECT id, nome, ordem
     FROM graduacoes
     WHERE categoria_id = ?
       AND organizacao_id = ?
     ORDER BY ordem ASC`, [categoriaId, organizacaoId]);
    return rows;
}
/* -------------------------------------------------------------------------- */
...
async function criar({ nome, ordem, categoriaId, organizacaoId }) {
    ...
    const [result] = await db.execute(`INSERT INTO graduacoes (nome, ordem, categoria_id, organizacao_id)
     VALUES (?, ?, ?, ?)`, [nome, ordem, categoriaId, organizacaoId]);
...
async function remover(id, organizacaoId) {
    ...
    const [result] = await db.execute(`DELETE FROM graduacoes 
      WHERE id = ? 
        AND organizacao_id = ?`, [id, organizacaoId]);
    return result.affectedRows > 0;
}
/* -------------------------------------------------------------------------- */
