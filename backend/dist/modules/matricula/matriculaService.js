...
const { gerarEmailMatriculaAprovada, gerarEmailMatriculaAprovadaAdmin, } = require("../../services/templates/matriculaAprovada");
...
async function criarMatricula(dados) {
    try {
        ...
        if (usuario?.organizacao_id && !dados.organizacao_id) {
            ...
            logger.debug(`[matriculaService] organizacao_id herdado do token → ${dados.organizacao_id}`);
        }
        if (!dados.organizacao_id) {
            logger.warn("[matriculaService] organizacao_id ausente, buscando fallback...");
        }
        ...
        if (!dados.organizacao_id) {
            ...
            logger.debug(`[matriculaService] organizacao_id herdado da turma → ${dados.organizacao_id}`);
        }
        if (!dados.organizacao_id)
        ...
        try {
            ...
            if (!dados.turma_id) {
                logger.warn(`[matriculaService] org ${dados.organizacao_id} - turma_id ausente antes do e-mail, tentando fallback...`);
                const turmaFallback = await matriculaRepository.buscarTurmaPorIdade(idade);
                if (turmaFallback) {
                    dados.turma_id = turmaFallback.turma_id;
                    logger.info(`[matriculaService] org ${dados.organizacao_id} - turma_id recuperado via fallback (${turmaFallback.turma_id}, ${turmaFallback.turma_nome})`);
                }
                else {
                    logger.error(`[matriculaService] org ${dados.organizacao_id} - nenhum turma_id encontrado mesmo após fallback`);
                }
            }
            logger.debug(`[matriculaService] org ${dados.organizacao_id} - preparando envio de e-mail (turma_id=${dados.turma_id || "N/A"})`);
            const dadosEmail = await matriculaRepository.buscarDadosEmailAprovacao(dados.turma_id, dados.organizacao_id);
            logger.debug("[matriculaService] Dados do e-mail de matrícula:", dadosEmail);
            // 🔹 E-mail do aluno
            if (dados.email) {
            ...
            const emailsAdmin = await notificacaoService.getEmails(dados.organizacao_id, "matricula");
            ...
            for (const email of emailsAdmin) {
                try {
                    ...
                    logger.warn(`[matriculaService] Falha ao enviar e-mail admin ${email}: ${err.message}`);
                }
            }
            logger.info(`[matriculaService] org ${dados.organizacao_id} - e-mails de matrícula enviados (aluno + admin)`);
        }
        catch (err) {
...
async function criarMatriculaDireta(pre) {
    ...
    logger.debug(`[matriculaService] org ${organizacao_id} - iniciando criação de matrícula direta para pré ${pre.id} (${pre.nome})`);
    try {
        // 🧩 Normalizações básicas
        ...
        if (turma) {
            ...
            logger.info(`[matriculaService] org ${organizacao_id} - turma ${turma.turma_id} atribuída automaticamente (${turma.turma_nome}, idade ${idade})`);
        }
        else {
            logger.warn(`[matriculaService] org ${organizacao_id} - nenhuma turma encontrada compatível com idade ${idade}`);
        }
        ...
        logger.debug(`[matriculaService] org ${organizacao_id} - payload final do aluno:`, dadosAluno);
        // 💾 Cria aluno
        const novoAluno = await matriculaRepository.criar(dadosAluno);
        logger.info(`[matriculaService] org ${organizacao_id} - aluno criado com ID ${novoAluno.id} a partir da pré ${pre.id}`);
        // 🔄 Atualiza status da pré-matrícula
        await preMatriculasRepository.atualizarStatus(pre.id, "aprovado", organizacao_id);
        // ✉️ Envio de e-mails (aluno + admin)
        // 🧩 Garante que o turma_id exista antes de montar o e-mail
        if (!pre.turma_id) {
            logger.warn(`[matriculaService] org ${organizacao_id} - turma_id ausente antes do e-mail, tentando fallback por idade...`);
            const turmaFallback = await matriculaRepository.buscarTurmaPorIdade(idade);
            if (turmaFallback) {
                pre.turma_id = turmaFallback.turma_id;
                logger.info(`[matriculaService] org ${organizacao_id} - turma_id recuperado via fallback (${turmaFallback.turma_id}, ${turmaFallback.turma_nome})`);
            }
            else {
                logger.error(`[matriculaService] org ${organizacao_id} - nenhuma turma encontrada mesmo após fallback (idade ${idade})`);
            }
        }
        logger.debug(`[matriculaService] org ${organizacao_id} - preparando envio de e-mail (turma_id=${pre.turma_id || "N/A"})`);
        const dadosEmail = await matriculaRepository.buscarDadosEmailAprovacao(pre.turma_id, pre.organizacao_id);
        logger.debug("[matriculaService] Dados do e-mail de matrícula direta:", dadosEmail);
        if (pre.email) {
            try {
                const htmlAluno = gerarEmailMatriculaAprovada({
                    ...pre,
                ...
                const emailsAdmin = await notificacaoService.getEmails(pre.organizacao_id, "matricula");
                const htmlAdmin = gerarEmailMatriculaAprovadaAdmin({
                    ...pre,
                    ...dadosEmail,
                });
                ...
                logger.info(`[matriculaService] org ${organizacao_id} - e-mails de matrícula aprovados enviados`);
            }
            catch (err) {
                logger.error(`[matriculaService] org ${organizacao_id} - erro ao enviar e-mails de matrícula aprovada: ${err.message}`);
            }
        }
        logger.info(`[matriculaService] org ${organizacao_id} - matrícula direta concluída com sucesso (pré ${pre.id} → aluno ${novoAluno.id})`);
        return novoAluno.id;
    }
    catch (err) {
        logger.error(`[matriculaService] org ${organizacao_id} - erro em criarMatriculaDireta: ${err.message}`);
        throw err;
    }
}
/**
...
async function enviarEmailRecusaMatricula(matricula) {
    try {
        ...
        logger.info(`[matriculaService] org ${matricula.organizacao_id} - e-mail de recusa enviado para ${matricula.email}`);
    }
    catch (err) {
