...
async function criar(dados) {
    logger.debug(`[matriculaRepository] Iniciando criação de aluno (${dados.nome}) para org ${dados.organizacao_id}`);
    const sql = `
  INSERT INTO alunos (
    organizacao_id,
...
`;
    ...
    `;
        ...
        logger.info(`[matriculaRepository] Matrícula criada com ID ${matriculaResult.insertId} para aluno ${alunoId}`);
    }
    else {
        logger.warn(`[matriculaRepository] Nenhuma turma atribuída — matrícula não criada (aluno ${alunoId})`);
    }
...
async function buscarTurmaPorIdade(idade, organizacao_id) {
    ...
    `;
        ...
        if (rows.length > 0) {
            logger.info(`[matriculaRepository] org ${organizacao_id} - turma compatível encontrada (${rows[0].turma_nome})`);
            return rows[0];
        }
        logger.warn(`[matriculaRepository] org ${organizacao_id} - nenhuma turma encontrada para idade ${idade}`);
        return null;
    }
    catch (err) {
        logger.error(`[matriculaRepository.buscarTurmaPorIdade] Erro: ${err.message}`);
        throw err;
    }
}
/**
...
async function buscarOrganizacaoPorTurmaId(turmaId) {
    const [rows] = await db.execute("SELECT organizacao_id FROM turmas WHERE id = ?", [turmaId]);
...
async function buscarDadosEmailAprovacao(turmaId, organizacaoId) {
  ...
  `;
    ...
    const row = rows[0];
    logger.info(`[matriculaRepository.buscarDadosEmailAprovacao] OK (turma_id=${turmaId}, org=${organizacaoId}) → ` +
...
async function buscarDadosOrganizacao(organizacaoId) {
  ...
  `;
    ...
    const [rows] = await db.execute(sql, params);
    if (!rows.length) {
        logger.warn(`[matriculaRepository.buscarDadosOrganizacao] Nenhuma organização encontrada (id=${organizacaoId})`);
        return null;
    }
    const row = rows[0];
    logger.info(`[matriculaRepository.buscarDadosOrganizacao] OK (org=${organizacaoId}) → ` +
