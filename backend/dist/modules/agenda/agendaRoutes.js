...
router.post("/upload-imagem", verifyToken, checkRole(["admin", "instrutor", "midia"]), upload.single("imagem"), agendaController.criarEventoComImagem);
// Criar evento
...
router.put("/:id/status", verifyToken, checkRole(["admin"]), agendaController.atualizarStatus);
// Arquivar evento (marca como concluído e limpa imagem)
router.put("/:id/arquivar", verifyToken, checkRole(["admin", "instrutor", "midia"]), agendaController.arquivarEvento);
/* -------------------------------------------------------------------------- */
