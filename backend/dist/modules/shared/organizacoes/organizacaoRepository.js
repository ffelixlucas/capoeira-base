...
async function buscarIdPorSlug(slug) {
    try {
        ...
        const [rows] = await db.execute("SELECT id FROM organizacoes WHERE slug = ? LIMIT 1", [slug]);
        if (!rows.length) {
            logger.warn(`[organizacaoRepository] Slug não encontrado: ${slug}`);
            return null;
        }
        logger.debug(`[organizacaoRepository] Slug "${slug}" resolvido → org ${rows[0].id}`);
        return rows[0].id;
    }
    catch (err) {
        logger.error("[organizacaoRepository] Erro ao buscar ID da organização por slug:", err.message);
        throw err;
    }
}
/* -------------------------------------------------------------------------- */
...
async function buscarPorSlug(slug) {
    try {
        if (!slug)
            throw new Error("Slug não informado.");
        const [rows] = await db.execute("SELECT * FROM organizacoes WHERE slug = ? LIMIT 1", [slug]);
        if (!rows.length) {
        ...
        logger.error("[organizacaoRepository] Erro ao buscar organização por slug:", err.message);
        throw err;
    }
}
/* -------------------------------------------------------------------------- */
...
async function buscarPorId(id) {
    try {
        const [rows] = await db.execute("SELECT * FROM organizacoes WHERE id = ? LIMIT 1", [id]);
        if (!rows.length) {
        ...
        logger.error("[organizacaoRepository] Erro ao buscar organização por ID:", err.message);
        throw err;
    }
}
module.exports = {
