...
exports.instrutorPossuiTurma = async (equipeId, turmaId) => {
    const [rows] = await db.execute("SELECT 1 FROM turmas WHERE id = ? AND equipe_id = ? LIMIT 1", [turmaId, equipeId]);
    return rows.length > 0;
};
/** Lista as turmas de um instrutor (responsável pelo espaço ou pela turma) */
exports.turmasDoInstrutor = async (equipeId) => {
    const [rows] = await db.execute("SELECT id, nome FROM turmas WHERE equipe_id = ? ORDER BY nome ASC", [equipeId]);
    return rows;
};
/** Lista presenças por turma e data (dia) */
exports.listarPorTurmaEData = async (turmaId, data, organizacaoId) => {
    const [rows] = await db.execute(`SELECT p.*, a.nome AS aluno_nome, a.apelido
       FROM presencas p
       JOIN alunos a ON a.id = p.aluno_id
      WHERE p.turma_id = ? AND p.data = ? AND p.organizacao_id = ?
      ORDER BY a.nome ASC`, [turmaId, data, organizacaoId]);
    return rows;
};
...
exports.upsertBatch = async (linhas = []) => {
    ...
    for (const l of linhas) {
        values.push(l.aluno_id, l.turma_id, l.data, l.status, l.created_by ?? null, l.organizacao_id ?? null // 🔹 novo campo
        );
    }
    const sql = `
...
exports.atualizarUma = async (id, { status, observacao }) => {
    ...
    if (fields.length === 0)
        return;
    vals.push(id);
    const sql = `UPDATE presencas SET ${fields.join(", ")}, updated_at = CURRENT_TIMESTAMP WHERE id = ?`;
