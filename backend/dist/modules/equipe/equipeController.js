...
async function atualizarEquipe(req, res) {
    try {
        const organizacaoId = req.usuario?.organizacao_id;
        const atualizado = await equipeService.atualizarEquipe(req.params.id, req.body, organizacaoId);
        if (atualizado) {
...
async function atualizarPerfil(req, res) {
    try {
        const organizacaoId = req.usuario?.organizacao_id;
        const atualizado = await equipeService.atualizarEquipe(req.usuario.id, req.body, organizacaoId);
        if (atualizado) {
...
async function alterarSenha(req, res) {
    try {
        ...
        const resultado = await equipeService.alterarSenha(req.usuario.id, senhaAtual, novaSenha, organizacaoId);
