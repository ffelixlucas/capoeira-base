...
const listarPorEvento = async (req, res) => {
    try {
        ...
        const dados = await inscricoesService.listarPorEvento(req.params.eventoId, busca, categoria);
...
async function extornarPagamento(req, res) {
    try {
        ...
        logger.error("❌ Erro no controller extornarPagamento:", error?.response?.data || error);
