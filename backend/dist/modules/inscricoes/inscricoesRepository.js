...
async function listarPorEvento(eventoId, busca = "", categoria = "todos") {
    // 1) Dados do evento
    const [eventoRows] = await db.execute(`SELECT id, titulo, valor, data_inicio 
     FROM agenda 
     WHERE id = ?`, [eventoId]);
...
`;
    ...
    const [camisetasRows] = await db.execute(`SELECT tamanho_camiseta AS tamanho, COUNT(*) AS total
       FROM inscricoes_evento
       WHERE evento_id = ? AND status = 'pago'
       GROUP BY tamanho_camiseta
       ORDER BY tamanho_camiseta`, [eventoId]);
...
async function criarInscricao(dados) {
    const { evento_id, nome, apelido = null, data_nascimento = null, email = null, telefone = null, cpf = null, autorizacao_participacao = null, autorizacao_imagem = null, documento_autorizacao_url = null, valor = null, responsavel_nome = null, responsavel_documento = null, responsavel_contato = null, responsavel_parentesco = null, tamanho_camiseta = null, alergias_restricoes = null, aceite_imagem = 0, aceite_responsabilidade = 0, aceite_lgpd = 0, } = dados;
    if (!evento_id || !nome) {
...
async function atualizarInscricaoParaExtornado(id, dados) {
    await db.execute(`UPDATE inscricoes_evento
       SET status = 'extornado',
           ...
           refund_data = NOW(),
           atualizado_em = NOW()
     WHERE id = ?`, [dados.refund_id ?? null, dados.refund_valor ?? null, id]);
}
/**
