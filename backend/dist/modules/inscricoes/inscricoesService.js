...
async function extornarPagamentoService(id) {
    ...
    try {
        ...
        const { data: result } = await axios.post(`https://api.mercadopago.com/v1/payments/${inscricao.pagamento_id}/refunds`, {}, // body vazio = estorno total
        {
            headers: {
        ...
        try {
            const { data: pagamento } = await axios.get(`https://api.mercadopago.com/v1/payments/${inscricao.pagamento_id}`, {
                headers: {
                    Authorization: `Bearer ${process.env.MERCADO_PAGO_ACCESS_TOKEN}`,
                },
            });
