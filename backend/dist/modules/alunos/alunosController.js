...
async function listar(req, res) {
    ...
    try {
        ...
        const alunos = await alunoService.listarTodos(usuario, turmaId, organizacaoId);
        res.json(alunos);
    }
    catch (err) {
...
async function trocarTurma(req, res) {
    try {
        ...
        if (!nova_turma_id)
            throw new Error("Nova turma obrigatória.");
...
async function metricasAluno(req, res) {
    try {
        ...
        if (!fim)
            fim = hoje;
        const metricas = await alunoService.metricasAluno(Number(id), inicio, fim, organizacaoId);
        res.json(metricas);
    }
    catch (err) {
...
async function listarPendentes(req, res) {
    try {
        const usuario = req.usuario;
        const pendentes = await alunoService.listarPendentes(usuario.organizacao_id);
        res.json(pendentes);
    }
    catch (err) {
...
async function atualizarStatus(req, res) {
    try {
        ...
        await alunoService.atualizarStatus(req.params.id, status, usuario.organizacao_id);
        res.json({ sucesso: true });
    }
    catch (err) {
