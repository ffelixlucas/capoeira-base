...
async function criarAluno(dados) {
    const { organizacao_id, nome, apelido, nascimento, telefone_responsavel, nome_responsavel, endereco, graduacao, observacoes_medicas, turma_id, } = dados;
    const sql = `
    INSERT INTO alunos (
  ...
  `;
    ...
    await connection.execute(`INSERT INTO matriculas (aluno_id, turma_id, organizacao_id, data_inicio) 
     VALUES (?, ?, ?, CURDATE())`, [alunoId, turma_id, organizacao_id]);
...
async function editarAluno(id, dados, organizacaoId) {
    ...
    await connection.execute(`UPDATE alunos SET ${sets} WHERE id = ? AND organizacao_id = ?`, valores);
...
async function excluirAluno(id, organizacaoId) {
    await connection.execute(`DELETE FROM alunos WHERE id = ? AND organizacao_id = ?`, [id, organizacaoId]);
}
/* -------------------------------------------------------------------------- */
...
async function contarPendentes(organizacaoId) {
    const [rows] = await connection.execute(`SELECT COUNT(*) AS total FROM alunos WHERE status = 'pendente' AND organizacao_id = ?`, [organizacaoId]);
